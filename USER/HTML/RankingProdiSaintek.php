...
<html>
    <head>
        <title>List Prodi Saintek</title>
    ...
    <div class="container">
        <br>
        <h2 style="text-align: center">List Program Studi Saintek</h2>
        <br>
        <table border="1">
            ...
            $list = 'SELECT id_prodisaintek, kode_ptn, prodi_ptn, utbk, dayatampung_baru, peminat_lama, ratio, 
peminat_baru, rankrendah_pendaftar, rankrendah_peluang FROM prodisaintek';
            $query = mysqli_query($conn, $list);
            while ($row = mysqli_fetch_array($query)){
                echo '<tr style="text-align: center">
                                <td>'.$row['id_prodisaintek'].'</td>
                                <td>'.$row['kode_ptn'].'</td>
                                <td>'.$row['prodi_ptn'].'</td>
                                <td>'.$row['utbk'].'</td>
